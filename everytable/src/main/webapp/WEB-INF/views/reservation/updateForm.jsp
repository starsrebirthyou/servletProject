...
<script type="text/javascript">
	$(function() {
		...
		$("#addMenuBtn").on("click", function() {
			...
			// [핵심수정] name="prices" 추가 (디자인 무너지지 않게 태그 구조 유지)
			const newRow = `
    <tr class="menu-row">
        ...
        <td class="text-center">
            ...
            <input type="hidden" name="price" class="unit-price" value="` + price + `">
        </td>
	...
	<div class="page-wrap">
		<div class="detail-card">
			<form action="update.do" method="post">
				<input type="hidden" name="resNo" value="${vo.resNo}"> <input
				...
				<div class="row" style="display: flex; gap: 40px;">
					<div style="flex: 1;">
						...
						<label class="small text-muted">매장명</label> <input type="text"
							class="form-control read-only-input" value="${vo.storeName}"
							readonly> <label class="small text-muted">연락처</label> <input
							type="text" name="resPhone" class="form-control"
							value="${vo.resPhone}"> <label class="small text-muted">방문
							예정 일시</label>
						<div style="display: flex; gap: 10px;">
							<input type="date" name="resDate" class="form-control"
								value="${vo.resDate}"> <input type="time" name="resTime"
								class="form-control" value="${vo.resTime}">
						</div>

						<label class="small text-muted">예약 인원</label> <input type="number"
							name="resCount" class="form-control" value="${vo.resCount}"
							min="1"> <label class="small text-muted">예약 유형</label> <select
							class="form-select" name="resType">
							<option value="단체" ${vo.resType == '단체'?'selected':''}>단체</option>
							<option value="픽업" ${vo.resType == '픽업'?'selected':''}>픽업</option>
						</select> <label class="small text-muted">요청 사항</label> <input type="text"
							name="orderAdd" class="form-control" value="${vo.orderAdd}">
					</div>
					...
					<div style="flex: 1;">
								...
								<c:forEach items="${vo.orderList}" var="item">
									<tr class="menu-row">
										...
										<td class="text-center"><input type="number"
											name="quantities" class="qty-input" value="${item.quantity}"
											min="0"
											style="width: 50px; text-align: center; border: 1px solid #ddd; border-radius: 6px;">
											<input type="hidden" name="menuNos" value="${item.menuNo}">
											<input type="hidden" name="price" class="unit-price"
											value="${item.price}"></td>
										<td class="text-end subtotal-text"><fmt:formatNumber
												value="${item.price * item.quantity}" />원</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>

						<div class="section-title" style="margin-top: 30px;">Add New
							Menu</div>
						<div style="display: flex; gap: 5px;">
							<select id="menuSelect" class="form-select"
								style="flex: 3; font-size: 13px;">
								<option value="">-- 추가할 메뉴 선택 --</option>
								<c:forEach items="${vo.storeMenuList}" var="menu">
									<option value="${menu.menu_no}" data-name="${menu.menu_name}"
										data-price="${menu.price}">${menu.menu_name}(
						...
						<div class="total-row">
							<span>최종 결제 금액</span> <span class="total-price"
								id="displayTotalPrice"> <fmt:formatNumber
									value="${vo.totalPrice}" />원
							</span>
						</div>
					</div>
				</div>
				...
				<div
					style="text-align: center; margin-top: 30px; border-top: 1px solid #f0f0f0; padding-top: 20px; display: flex; justify-content: center; gap: 10px;">
					<button type="submit" class="btn-custom btn-primary-custom">수정
						완료</button>
