...
.card-meta { font-size: 12px; color: #aaa; margin-bottom: 4px; }
.card-user { font-size: 14px; color: #444; }
.card-user strong { color: #111; font-weight: 600; }
...
.badge-etc   { background: #f3f4f6; color: #6b7280; border: 1px solid #e5e7eb; }
...
.info-cell .val.blue  { color: #1d4ed8; }
...
.cancel-reason strong { font-weight: 600; }
...
.modal-box textarea:focus { outline: none; border-color: #111; background: #fff; }
...
<div class="page-wrap">
    ...
    <c:forEach items="${list}" var="vo">
        <div class="order-card">
            ...
            <div class="card-top">
                <div>
                    <div class="card-meta">No.${vo.resNo} &nbsp;·&nbsp; 접수 ${vo.resCreatedAt}</div>
                    <div class="card-user">
                        <strong>${vo.userId}</strong> &nbsp;·&nbsp; ${vo.resPhone}
                    </div>
                </div>
                <c:choose>
                    <c:when test="${vo.resStatus == 1}"><span class="badge badge-wait">대기</span></c:when>
                    <c:when test="${vo.resStatus == 2}"><span class="badge badge-ok">승인</span></c:when>
                    <c:when test="${vo.resStatus == 3}"><span class="badge badge-done">완료</span></c:when>
                    <c:when test="${vo.resStatus == 4}"><span class="badge badge-cancel">취소·환불</span></c:when>
                    <c:otherwise><span class="badge badge-etc">알 수 없음</span></c:otherwise>
                </c:choose>
            </div>
            ...
            <div class="card-actions">
                ...
                <c:if test="${vo.resStatus == 1}">
                    ...
                    <button type="button"
                            class="btn btn-danger btn-reject-modal"
...
<div class="modal-overlay" id="rejectModal">
    <div class="modal-box">
        ...
        <form action="updateStatus.do" method="post">
            ...
            <textarea name="cancelReasonDirect" rows="3" placeholder="기타 사유 입력 시 작성"></textarea>
            <div class="modal-actions">
                <button type="button" class="btn btn-ghost" id="modalClose">취소</button>
                <button type="submit" class="btn btn-danger" style="background:#b91c1c; color:#fff; border-color:#b91c1c;">거절 처리</button>
            </div>
        </form>
    </div>
</div>
...
(function(){
    ...
    document.querySelectorAll("#filterKey .filter-btn").forEach(function(btn){
        ...
        btn.addEventListener("click", function(){
            location.href = "adminList.do?page=1&perPageNum=" + perPageNum + "&key=" + btn.value;
        });
    });

    document.querySelectorAll(".btn-reject-modal").forEach(function(btn){
        btn.addEventListener("click", function(){
            document.getElementById("modalResNo").value = btn.dataset.no;
