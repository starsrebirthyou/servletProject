...
<body>
	...
	<div class="page-wrap">
		...
		<c:forEach items="${list}" var="vo">
			<div class="order-card" data-no="${vo.resNo}">
				...
				<div class="card-top">
					...
					<c:choose>
						<c:when test="${vo.resStatus == 1}">
							<span class="badge badge-wait">매장 확인 중</span>
						</c:when>
						<c:when test="${vo.resStatus == 2}">
							<span class="badge badge-ok">예약 확정</span>
						</c:when>
				...
				<c:choose>
					...
					<c:otherwise>
						<div class="step-bar">
							<span class="step-item">대기</span> <span
								class="step-arrow ${vo.resStatus >= 2 ? '' : 'dim'}">▶</span> <span
								class="step-item ${vo.resStatus >= 2 ? '' : 'dim'}">승인</span> <span
								class="step-arrow ${vo.resStatus >= 3 ? '' : 'dim'}">▶</span> <span
								class="step-item ${vo.resStatus >= 3 ? '' : 'dim'}">완료</span>
						</div>
					</c:otherwise>
				</c:choose>
		...
		<div class="bottom-actions">
			<a href="writeForm.do" class="btn btn-dark">새 예약하기</a> <a
				href="list.do" class="btn btn-ghost">목록 새로고침</a>
		</div>
			...
			document.querySelectorAll("#filterKey .filter-btn").forEach(
					function(btn) {
						...
						btn.addEventListener("click", function() {
							location.href = "list.do?page=1&perPageNum="
									+ perPageNum + "&key=" + btn.value;
						});
					});

			document.querySelectorAll(".order-card").forEach(
					function(card) {
						card.addEventListener("click", function() {
							location.href = "view.do?no=" + card.dataset.no
									+ "&" + pageQuery;
						});
					});
		})();
