...
public class ReservationDAO extends DAO {
	...
	public List<ReservationVO> list(PageObject pageObject) throws Exception {
		...
		String sql = "select m.store_name, r.res_no, to_char(r.res_date, 'yyyy-mm-dd') res_date, "
				...
				+ " from reservation r, member m " + " where (r.user_id = m.id) ";
	...
	public ReservationVO view(Long no) throws Exception {
	    ...
	    // 조인(Join)을 빼고 한 테이블에서 필요한 걸 다 가져옵니다.
	    // 쿼리에서 r.store_id 같은 조인 조건을 지웠습니다.
	    String sql = "select store_name, user_id, res_phone, "
	            + " to_char(res_date, 'yyyy-mm-dd') res_date, res_time, "
	            + " res_count, res_type, res_no, total_price, res_status, cancel_reason " 
	            + " from reservation "
	            + " where res_no = ?";
	    ...
	    if (rs != null && rs.next()) {
	        ...
	        vo.setResNo(rs.getLong("res_no"));
	        vo.setStoreName(rs.getString("store_name")); // r. 없이 컬럼명만!
	        ...
	        vo.setResStatus(rs.getLong("res_status"));
	        vo.setCancelReason(rs.getString("cancel_reason")); // 취소 사유 추가
	    }
	    DB.close(con, pstmt, rs);
	    return vo;
	}
	...
	public int cancel(ReservationVO vo) throws Exception {
		...
		try {
			con = DB.getConnection();
			// 삭제(DELETE)가 아니라 상태 업데이트(UPDATE)!
			String sql = "update reservation set res_status = ?, cancel_reason = ? where res_no = ?";

			pstmt = con.prepareStatement(sql);
			pstmt.setLong(1, vo.getResStatus()); // 4
			pstmt.setString(2, vo.getCancelReason()); // 취소 사유
			pstmt.setLong(3, vo.getResNo()); // 예약 번호
